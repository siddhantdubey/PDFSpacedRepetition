...
export default function PDFViewer() {
  ...
  const [highlights, setHighlights] = useState<{ [key: number]: { content: string; text: string }[] }>({});
  ...
  const [annotationPosition, setAnnotationPosition] = useState<{ x: number; y: number; width: number; height: number } | null>(null);
  const [highlightPage, setHighlightPage] = useState<number | null>(null);
  const [annotationBoxPosition, setAnnotationBoxPosition] = useState<{ top: number; left: number } | null>(null);
  const [hoveredAnnotation, setHoveredAnnotation] = useState<{ page: number; index: number } | null>(null);
  ...
  const fitToScreen = async () => {
    ...
    const newScale = scaledWidth > containerWidth 
    ...
    setPageSize({ width: viewport.width * newScale, height: viewport.height * newScale });
  };

  const onItemClick = ({ pageNumber }: { pageNumber: string | number }) => {
    const targetPage = typeof pageNumber === 'string' ? parseInt(pageNumber, 10) : pageNumber;
    ...
    if (selection && selection.toString().trim() !== '') {
      ...
      if (rects.length > 0) {
        ...
        const lineGroups = rects.reduce((acc, rect) => {
          const group = acc.find(g => Math.abs(g[0].top - rect.top) < threshold);
          if (group) {
            group.push(rect);
          } else {
            acc.push([rect]);
        ...
        const annotationPosition = {
          ...
          width: Math.max(...optimalRects.map(r => r.x + r.width)) - Math.min(...optimalRects.map(r => r.x)),
          height: Math.max(...optimalRects.map(r => r.y + r.height)) - Math.min(...optimalRects.map(r => r.y)),
        };
      ...
      annotation.rects.map((rect, rectIndex) => (
        <div
          ...
          style={{
            ...
            backgroundColor: 
              (hoveredAnnotation?.page === pageNumber && hoveredAnnotation?.index === index) ||
              (hoveredHighlight?.page === pageNumber && hoveredHighlight?.index === index)
    ...
    <div className="bg-white font-sans min-h-screen flex flex-col">
      <header className="bg-[#323639] shadow-md p-5 text-white">
        ...
        <div className="mb-4 flex gap-2 items-center">
          ...
          <span className="text-white px-4 py-2">{Math.round(scale * 100)}%</span>
      ...
      <div className="flex flex-grow overflow-hidden">
              ...
              {pageAnnotations.map((annotation, index) => (
                <div 
                  ...
                  className={`mb-4 p-2 rounded-md shadow cursor-pointer ${
                    (hoveredAnnotation?.page === Number(pageNumber) && hoveredAnnotation?.index === index) ||
                    (hoveredHighlight?.page === Number(pageNumber) && hoveredHighlight?.index === index)
        ...
        <div className="flex-grow overflow-auto relative" ref={setContainerRef}>
              ...
              itemContent={(index) => (
                ...
                >
                  <Page
                    ...
                    loading={<div className="text-white">Loading page {index + 1}...</div>}
          ...
          {highlightedText && annotationBoxPosition && createPortal(
            <div 
              className="fixed bg-white p-4 shadow-md rounded-md"
              style={{
